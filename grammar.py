...
class MemoryAnalysisHelper:
    def __init__(self):
        ...
        self.ffi.cdef("""
            ...
            void free_helper(void* helper);
            void print_top_n_weights(void* helper, size_t n);
        """)
    ...
    def add_creator_line(self, orginal_rule: str):
        self.Clib.add_creator_line(self.inner,
                                   self.ffi.new("char []", orginal_rule.encode("utf-8")))

    def prepared(self):
        self.Clib.prepared(self.inner)

    def update(self, rule_name: str):
        self.Clib.update_weights(self.inner, self.ffi.new("char []", rule_name.encode("utf-8")))

    def random_choice_with_weights(self) -> int:
        lineno = self.Clib.random_choice_with_weights(self.inner)
        return lineno

    def print_top_n_weights(self, n):
        self.Clib.print_top_n_weights(self.inner, n)
...
# "statement_map": {statement1: 0, statement2: 1},
# "graph": {statement1_index: {statement2_index, statement3_index}, ...}
# }
def _get_mem_dep_from_file():
    mem_dep = {}
    stm_map = {}
    mem_dep_path = os.getenv("MEM_DEP_JSON_PATH")
    if mem_dep_path is None:
        return None
    with open(mem_dep_path) as f:
        mem_dep = json.load(f)
    return mem_dep
...
class Grammar(object):
    ...
    def __init__(self, is_js=False, should_remove_redundant=False, use_mem_dep=False):
        ...
        self.use_mem_dep = use_mem_dep
        self.mem_dep_helper = None
        self.dump_js_seq = False
        if os.getenv("ENABLE_RULE_MAP") is not None:
            self.dump_js_seq = True
        self.last_generated_seq = []
        if is_js and use_mem_dep:
            self.mem_dep_helper = MemoryAnalysisHelper()
    ...
    def _generate_code(self, num_lines, initial_variables=[], last_var=0):
        ...
        if self.is_js and self.use_mem_dep:
            self.mem_dep_helper.reset_weights()
        ...
        while len(context['lines']) < num_lines:
            ...
            try:
                if self.is_js and self.use_mem_dep:
                    ...
                    lineno = self.mem_dep_helper.random_choice_with_weights()
                else:
    ...
    def _expand_rule(self, symbol, rule, context,
        ...
        if rule['type'] == 'grammar':
            ...
            if self.is_js and self.dump_js_seq and rule in self._creators['line']:
                self.last_generated_seq.append(rule["original_line"])

            if self.is_js and self.use_mem_dep:
                self.mem_dep_helper.update(rule["original_line"])
...
``
    ...
    def _prepare_for_mem_dep(self):
        for rule in self._creators['line']:
            original_line_statement = rule["original_line"]
            self.mem_dep_helper.add_creator_line(original_line_statement)
        self.mem_dep_helper.prepared()

    def _update_mem_dep_context(self, context, last_select_lineno):
        for read_lineno in self.creator_mem_dep["graph"][last_select_lineno]:
            index = context["lineno2index_map"][read_lineno]
            context["mem_interesting_weights"][index] += 1

        # tmp = [(context["mem_interesting_weights"][i], val)
        #        for i, val in enumerate(self._all_nonhelper_lines)]
    ...
    def remove_circle(self):
        ...
        for key in self._creators.keys():
            if self.can_reach_non_terminated(key, set(), res):
    ...
    def can_reach_non_terminated(self, current_tag, visited, res):
                ...
                if part["type"] == "tag":
                    ...
                    if part["tagname"] == current_tag:
                        if "new" not in part or not part["new"]:
                            ...
                            continue
                    if not self.can_reach_non_terminated(part["tagname"], visited, res):
                        inner_flag = False
                        break
            if inner_flag:
                flag = True
