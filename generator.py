...
def generate_samples(grammar_dir, outfiles):
    ...
    err = jsgrammar.parse_from_file(os.path.join(grammar_dir, 'js2.txt'))
    # exit()
...
class Generator:
    def __init__(self):
        ...
        if err > 0:
            print('There were errors parsing grammar')
            return
        if os.getenv("MEM_DEP_JSON_PATH") is None:
            print("please set MEM_DEP_JSON_PATH")
            exit(0)
        self.jsgrammar = Grammar(is_js=True, should_remove_redundant=True, use_mem_dep=True)
    ...
    def generate_one(self) -> str:
        result = generate_new_sample(self.template, self.htmlgrammar, self.cssgrammar,
                                     self.jsgrammar)
        print("successfully generate a html file")
        return result
